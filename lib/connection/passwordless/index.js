...
var _dataFns = dataFns(["passwordless"]),
    get = _dataFns.get,
  ...
  if (opts.defaultLocation && typeof opts.defaultLocation === "string") {
    ...
    m = sync(m, "location", {
      ...
      syncFn: function syncFn(m, cb) {
        return webAPI.getUserCountry(l.id(m), cb);
      },
      successFn: function successFn(m, result) {
        return initLocation(m, result);
      }
    });
  }
...
export function passwordlessConnection(m) {
  return l.connections(m, "passwordless", "email").get(0) || l.connections(m, "passwordless", "sms").get(0) || new Map();
}
